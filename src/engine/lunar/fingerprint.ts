...
export function lunarFingerprint(
  ...
  const resilience = finalState.metrics.morale > 0.6 ? 'robust' : finalState.metrics.morale > 0.35 ? 'stable' : 'fragile';
  const sustainment = finalState.metrics.foodMonthsReserve > 12 ? 'self-sustaining' : finalState.metrics.foodMonthsReserve > 6 ? 'resupply-dependent' : 'critical';
