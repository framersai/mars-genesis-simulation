...
export function marsDepartmentPromptLines(dept: string, state: SimulationState): string[] {
  ...
  switch (dept) {
    ...
    case 'engineering':
      lines.push('INFRASTRUCTURE:', `Modules: ${state.metrics.infrastructureModules} | Power: ${state.metrics.powerKw}kW | Life support: ${state.metrics.lifeSupportCapacity}/${state.metrics.population} | Volume: ${state.metrics.pressurizedVolumeM3}m³ | Water: ${state.metrics.waterLitersPerDay}L/day`);
      break;
    case 'agriculture':
      lines.push('FOOD:', `Reserves: ${state.metrics.foodMonthsReserve.toFixed(1)}mo | Pop to feed: ${state.metrics.population} | Farm modules: ${Math.floor(state.metrics.infrastructureModules * 0.3)}`);
      break;
    case 'psychology': {
      ...
      lines.push('PSYCH:', `Morale: ${Math.round(state.metrics.morale * 100)}% | Avg psych: ${avgPsych.toFixed(2)} | Depressed: ${depressed}/${alive.length} | Mars-born: ${alive.filter(c => c.core.marsborn).length}`);
