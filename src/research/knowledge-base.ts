/**
 * Topic-indexed research knowledge base.
 *
 * All DOI-linked citations reorganized by topic instead of turn number.
 * The Crisis Director generates crises with category tags, and matching
...
/** Map crisis category to relevant research topics. */
const CATEGORY_TOPICS: Record<string, ResearchTopic[]> = {
...
/** Get merged research packet for a crisis category and optional keywords. */
export function getResearchForCategory(category: string, keywords: string[] = []): CrisisResearchPacket {
  const topics = CATEGORY_TOPICS[category] || ['infrastructure'];
  ...
  for (const topic of topics) {
    const packet = KNOWLEDGE_BASE[topic];
    if (!packet) continue;
    for (const f of packet.canonicalFacts) {
      if (!seen.has(f.claim)) { seen.add(f.claim); facts.push(f); }
    }
    for (const c of packet.counterpoints) {
      if (!seen.has(c.claim)) { seen.add(c.claim); counters.push(c); }
    }
    for (const [dept, note] of Object.entries(packet.departmentNotes)) {
  ...
  return { canonicalFacts: facts.slice(0, 6), counterpoints: counters.slice(0, 3), departmentNotes: notes };
}

/** Legacy: get research by turn number (backward compat for static scenarios). */
export { getResearchPacket } from './research.js';
