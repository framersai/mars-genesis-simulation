...
test('marsFingerprint classifies high-morale colony as antifragile', () => {
  const result = marsFingerprint(
    { metrics: { morale: 0.75 }, politics: { earthDependencyPct: 30 }, agents: [{ health: { alive: true }, core: { marsborn: true } }, { health: { alive: true }, core: { marsborn: false } }] },
    [{ turn: 1, time: 2035, outcome: 'risky_success' }, { turn: 2, time: 2037, outcome: 'risky_success' }],
...
test('marsFingerprint classifies low-morale colony as brittle', () => {
  const result = marsFingerprint(
    { metrics: { morale: 0.2 }, politics: { earthDependencyPct: 80 }, agents: [{ health: { alive: true }, core: { marsborn: false } }] },
    [{ turn: 1, time: 2035, outcome: 'conservative_success' }, { turn: 2, time: 2037, outcome: 'conservative_success' }],
...
test('marsFingerprint identity is Martian when Mars-born > 30%', () => {
  ...
  const result = marsFingerprint(
    { metrics: { morale: 0.5 }, politics: { earthDependencyPct: 50 }, agents },
